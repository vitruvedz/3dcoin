...
unsigned int GetNextWorkRequiredv2(const CBlockIndex* pindexLast, const CBlockv2Header *pblockv2, const Consensus::Params& params)
{
    unsigned int retarget = DIFFV2_DGW;
    unsigned int nProofOfWorkLimit = UintToArith256(params.powLimit).GetCompact();

    if (pindexLast->nHeight+1 > Params().GetConsensus().nV014v1Start && pindexLast->nHeight+1 < Params().GetConsensus().nV014v1Start+10000 && pblockv2->GetBlockTime() > pindexLast->GetBlockTime() + params.nPowTargetSpacing*2) {
    ...
    // testnet -- we want a lot of coins in existance early on 
        ...
        if (pindexLast->nHeight + 1 >= 3000) retarget = DIFFV2_DGW;
        else retarget = DIFFV2_BTC;
    }

    // Default Bitcoin style retargeting
    if (retarget == DIFFV2_BTC)
    {
        ...
        if ((pindexLast->nHeight+1) % params.DifficultyAdjustmentInterval() != 0)
        {
            if (params.fPowAllowMinDifficultyBlocks)
            {
                ...
                if (pindexLast->nHeight+1 < 9500 && pblockv2->GetBlockTime() > pindexLast->GetBlockTime() + params.nPowTargetSpacing*2)
                    return nProofOfWorkLimit;
                else
                {
    ...
    else if (retarget == DIFFV2_KGW)
    {
        return KimotoGravityWell(pindexLast, params);
    }

    // Retarget using Dark Gravity Wave 3
    else if (retarget == DIFFV2_DGW)
    {
