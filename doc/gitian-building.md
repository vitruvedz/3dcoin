...
*Setup instructions for a Gitian build of 3DCoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the 3DCoin
...
- [Building 3DCoin Core](#building-3dcoin-core)
...
Clone the git repositories for 3DCoin Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/BlockchainTechLLC/3dcoin
...
in the 3DCoin Core repository under 'Fetch and build inputs' to install sources which require
...
To build 3DCoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the 3DCoin Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/3dcoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/BlockchainTechLLC/3dcoin
...
For example:
```bash
URL=https://github.com/BlockchainTechLLC/3dcoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit 3dcoin=${COMMIT} --url 3dcoin=${URL} ../3dcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit 3dcoin=${COMMIT} --url 3dcoin=${URL} ../3dcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit 3dcoin=${COMMIT} --url 3dcoin=${URL} ../3dcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the 3dcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../3dcoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/3dcoin.git
SIGPATH=/some/root/path/3dcoin-detached-sigs.git

./bin/gbuild --url 3dcoin=${BTCPATH},signature=${SIGPATH} ../3dcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/3dcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/3dcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/3dcoin-osx-build.assert
```
...
[3dcoin/gitian.sigs](https://github.com/BlockchainTechLLC/gitian.sigs/) repository, or if that's not possible to create a pull
request.
