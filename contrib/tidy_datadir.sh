...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete 3DCoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no 3DCoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old 3DCoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected 3DCoin 0.7 datadir."
    ;;
  3)
    echo "Detected 3DCoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected 3DCoin 0.8 datadir."
    ;;
esac
